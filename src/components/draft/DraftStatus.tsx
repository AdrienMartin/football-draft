...
export function DraftStatus({
  ...
  return (
    <section className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-2xl shadow-black/20 backdrop-blur">
      <h2 className="text-2xl font-semibold text-white">Draft joueur contre IA</h2>
      <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300">
        Chaque équipe choisit 5 joueurs à tour de rôle. L’IA prend le meilleur joueur
        disponible selon sa note.
      </p>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2">
        <div className="rounded-2xl border border-white/10 bg-slate-950/40 p-4">
          ...
          <p className="mt-2 text-lg font-semibold text-white">
            {isComplete ? 'Draft terminée' : currentTurn === 'user' ? 'À toi de choisir' : 'Tour de l’IA'}
          </p>
        </div>
        <div className="rounded-2xl border border-white/10 bg-slate-950/40 p-4">
          <p className="text-xs uppercase tracking-[0.2em] text-slate-400">Dernier choix</p>
          <p className="mt-2 text-lg font-semibold text-white">
            {lastPick ? `${lastPick.player.name} (${lastPick.team.toUpperCase()})` : 'Aucun choix'}
          </p>
        </div>
      </div>
    </section>
